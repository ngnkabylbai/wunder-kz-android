...
class TestActivity : AppCompatActivity() {
    ...
    private fun isTimeToEnd(): Boolean {
        if(currentQuestion >= questions.size){
            val builder = AlertDialog.Builder(this)
                    .setTitle("Поздравления")
                    .setMessage("Вы завершили тест.")
                    .setPositiveButton("OK", DialogInterface.OnClickListener { dialog, which ->

                        val resultIntent = Intent()
                        val resultText = "$rightAnswerCount/${questions.size}"
                        resultIntent.putExtra("result", resultText)
                        resultIntent.putExtra("seconds", secondsPassed)
    ...
    override fun onBackPressed() {
        val builder = AlertDialog.Builder(this)
                .setTitle("Внимание")
                .setMessage("Текущие данные не сохранятся. Вы точно хотите прервать тест?")
                .setPositiveButton("OK", DialogInterface.OnClickListener { dialog, which ->
                    finish()
                })
